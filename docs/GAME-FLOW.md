...
isComplete → true
    ...
    ↓（1700ms 後：COMPLETION_ANIM_DURATION_MS + 200ms 緩衝）
...
拼完最後一片 → `usePointerDrag` 觸發 `dispatch(setComplete(elapsed))` → `PuzzleBoard` 播放 **掃光動畫**（約 1.5 秒，期間封鎖互動）→ 動畫結束後顯示 `CompletionOverlay`。
