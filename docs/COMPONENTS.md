...
| `AppLayout` | `src/features/layout/AppLayout.tsx` | 所有路由的共用 layout（`<Outlet />`）；掛載 `usePreventForwardNav`、`useBackgroundMusic`、全域按鈕 click 音效；啟動時呼叫 `migrateToImageCache()`（同步）與 `initImageCache()`（非同步，將 IDB 圖片載入 `_mem`）；渲染 `AppHeader` |
...
| `imageCache.ts` | `initImageCache()`, `saveImage()`, `getImage()`, `pruneImageCache()`, `migrateToImageCache()` | 圖片快取層；以 IndexedDB（`puzzle-image-db`）儲存，啟動時載入至 `_mem` 供同步讀取；詳見 [RECORDS.md](RECORDS.md) |
