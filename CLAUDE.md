...
- key：`puzzle-game-history`，固定 10 元素陣列 `(GameHistoryRecord | null)[]`，空槽為 null
...
所有 phase 均顯示。右側常駐一顆**音量按鈕**（`ic-volume` / `ic-volume-off`，靜音時 opacity 0.65），點擊開啟 `VolumeModal`；`onClose` callback 同步更新 Header 的 muted state。`playing` phase 額外顯示難度、格數、計時器與操作按鈕（查看參考圖、暫停/繼續、保存並結束、結束遊戲）。
...
## 音效系統（`src/lib/soundEngine.ts`）

模組層級單例（module-level singleton），零外部音頻檔，完全以 Web Audio API 合成。

### SFX（音效）

| 函式 | 說明 |
|------|------|
| `playPickup()` | 抓起拼圖片（350Hz triangle，短促） |
| `playMerge()` | 兩片合組（440+880Hz 疊加） |
| `playSnap()` | 吸附至正確位置（523+784Hz） |
| `playComplete()` | 拼圖完成（C5→E5→G5→C6 四音琶音） |
| `playClick()` | 按鈕點擊（900Hz triangle，極短促） |

SFX 呼叫點：`usePointerDrag.ts`（`pointerdown` 時 `playPickup`；`pointerup` 結束後依合組/吸附/完成依序觸發，用 if/else if 確保同一次放下最多一個音效）。`playClick()` 由 `App.tsx` 的全域 **capture-phase** click listener 呼叫（`addEventListener('click', fn, true)`），對所有 `button` 元素生效，可繞過元件內的 `stopPropagation`。
...
**`toGain(vol)`**：`vol * 2`；slider 值 0–1 對應 gain 0–2，slider=0.5 時 gain=1.0（等於原始音量），slider=1.0 時 gain=2.0（放大一倍）。UI 以 0–100% 步進 10% 顯示，內部儲存 0.0–1.0。

匯出的音量 API：`getMusicVolume()`, `getButtonVolume()`, `getPieceVolume()`, `setMusicVolume(vol)`, `setButtonVolume(vol)`, `setPieceVolume(vol)`。
...
掛載於 `App.tsx`（`usePhaseHistory` 之後）。mount 時呼叫 `startMusic()`，unmount 時呼叫 `stopMusic()`；不監聽 `isPaused`，音樂跨所有 phase 持續播放。另監聽 `visibilitychange`：頁面隱藏時呼叫 `pauseForBackground()`，頁面恢復時呼叫 `resumeFromBackground()`（已靜音則跳過）。
...
- **瀏覽器返回導航**（`src/features/game/usePhaseHistory.ts`，掛載於 `App.tsx`）：每次向前進入非 home phase 時各 push 一筆 `{ puzzle: true, phase }` + `'#app'` URL；`popstate` 時比較 `storedPhase` 與 `currentPhase` 在 `PHASE_ORDER` 中的 index——backward（storedIdx < currentIdx）→ dispatch 對應 action；forward / stale 舊 session entry（storedIdx > currentIdx）→ `isUndoingForward` flag + `history.back()` 連續略過直到合法 entry；無 puzzle state（null entry）→ 若 phase 為 `playing` 則觸發返回攔截（見下），否則 `dispatch(goToHome())`。`isBackNav` flag 確保 back 觸發的 phase 變化不重複 push。phase 轉為 `home` 時有兩條清理路徑，防止殘留的 puzzle entries 讓使用者需要多按一次返回才能離開 app：(1) **非 back nav 路徑**（`resetGame`、保存並結束、結束確認）：`replaceState(null, '')` 取代 stale entry，若之前位於 puzzle entry（`hadPuzzleEntry=true`），設 `isUndoingForward=true` + `history.back()` 觸發清理鏈；(2) **isBackNav 路徑**（playing 攔截後返回、complete back nav）：`isBackNav` 重置後，若 `history.state?.puzzle` 仍為 truthy，同樣啟動清理鏈。各 phase 元件的返回按鈕（`ImageUpload`、`DifficultySelector`、`CropPreview`）一律呼叫 `history.back()` 而非直接 dispatch，行為與手勢返回完全一致。
- **Playing phase 返回攔截**：`playing` case 改為 `history.pushState(...)` 把 history 推回原位，然後呼叫 `opts?.onInterceptBackFromPlaying?.()`（**不 dispatch、不設 isBackNav**）。回呼存於 `interceptCallbackRef`（每 render 同步）以避免 stale closure，同時不造成 useEffect 重複註冊。**直接從首頁進入遊戲**（快速開局 / 讀取存檔）只 push 一筆 `{playing}`，前一筆為 null entry；`popstate` 落在 null entry 時同樣觸發此攔截邏輯（`!state?.puzzle` 分支中的 `phaseRef.current === 'playing'` 判斷），確保攔截行為與經過完整新遊戲流程（upload → playing）進入時一致。`App.tsx` 的 `handleExitRequest` 實作：`saveNow()` → `dispatch(pauseGame())` → `setShowExitConfirm(true)`；confirm 時 `dispatch(resetGame())`（**不 clearDraft**，讓草稿保留供首頁「繼續上局」）；cancel 時 `dispatch(resumeGame())`。
...
- `src/components/VolumeModal.tsx`：音量設定 Modal；頂部「全部靜音」toggle switch（`accent-500` 背景，控制全域靜音）；靜音時三個 range 均 disabled + opacity 降低；三個分類滑桿（背景音樂、按鈕音效、拼圖片，`step={10}`，0–100%，50%=原始音量）；即時預覽（調音樂呼叫 `startMusic()`，調按鈕播 `playClick()`，調拼圖片播 `playSnap()`）
- `src/features/upload/RecordsModal.tsx`：雙模式清單 Modal（`mode='quick'` 顯示快捷設定；`mode='history'` 顯示歷史紀錄固定 10 槽，空槽顯示虛線框）；各筆紀錄可刪除（需 `ConfirmDialog` 確認）；quick 模式的 Header 有「匯入代碼」按鈕，每筆卡片有「分享」按鈕
...
| `src/lib/soundEngine.ts` | `playPickup`, `playMerge`, `playSnap`, `playComplete`, `playClick`, `setMuted`, `isMuted`, `startMusic`, `stopMusic`, `pauseForBackground`, `resumeFromBackground`, `getMusicVolume`, `getButtonVolume`, `getPieceVolume`, `setMusicVolume`, `setButtonVolume`, `setPieceVolume` | 音效 SFX + 程序化鋼琴背景音樂引擎；靜音與三種音量分類以 `localStorage` 持久化；詳見音效系統說明 |
