...
SFX 呼叫點：`usePointerDrag.ts`（`pointerdown` 時 `playPickup`；`pointerup` 結束後依合組/吸附/完成依序觸發，用 if/else if 確保同一次放下最多一個音效）。
...
掛載於 `App.tsx`（`usePhaseHistory` 之後）。mount 時呼叫 `startMusic()`，unmount 時呼叫 `stopMusic()`；不監聽 `isPaused`，音樂跨所有 phase 持續播放。
...
| `src/lib/soundEngine.ts` | `playPickup`, `playMerge`, `playSnap`, `playComplete`, `setMuted`, `isMuted`, `startMusic`, `stopMusic` | 音效 SFX + 程序化鋼琴背景音樂引擎；靜音狀態以 `localStorage`（key: `puzzle-sound-muted`）持久化；詳見下方音效系統說明 |
