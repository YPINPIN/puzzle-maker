...
| 欄位 | 型別 | 說明 |
|------|------|------|
| `imageDataUrl` | `string \| null` | 原始上傳圖片（未裁切），僅在 upload→crop 階段使用 |
| `referenceDataUrl` | `string \| null` | 裁切後的參考圖，遊戲中「查看參考圖」使用 |
| `cropRegion` | 物件 | 相對於原圖的裁切座標（x, y, width, height） |
...
**圖片來源**：`handleConfirm` 傳入已裁切的 `croppedImageDataUrl`（≤800px JPEG）而非原始大圖 + `cropRegion`，與歷史紀錄 / 快捷設定路徑保持一致，避免手機對超大圖片的記憶體問題。
...
- **返回手勢攔截**：`App.tsx` 在 `phase === 'playing'` 時呼叫 `history.pushState({ puzzle: true }, '')` 建立假 history entry，監聽 `popstate` 攔截 Android/iOS back swipe，立即重新 `pushState` 並顯示 `ConfirmDialog` 確認是否結束遊戲，防止進度意外丟失
